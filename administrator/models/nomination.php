 ...
 * @since  1.6
 */
class GavotingModelNomination extends AdminModel
{
	/**
	 ...
	 * @since   3.2
	 */
	public $typeAlias = 'com_gavoting.nomination';
	...
	public function getForm($data = array(), $loadData = true)
	{
            ...
            // Get the form.
            $form = $this->loadForm(
                    'com_gavoting.nomination', 'nomination',
                    array('control' => 'jform',
                            'load_data' => $loadData
                    )
            );
	...
	/**
	 * Reset votes counter to zero
	 * @return mixed Array of data items on success, false on failure.
	 */
	public function resetZero(&$pks)
	{
		$table = $this->getTable();
	...
	protected function prepareTable($table)
	{
		if (empty($table->id))
		{
			...
			{
				$db = Factory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__gavoting_nominations');
				$max             = $db->loadResult();
