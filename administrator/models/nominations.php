...
class GavotingModelNominations extends ListModel
{
	...
	*/
	...
	protected function populateState($ordering = null, $direction = null)
	{
        ...
        parent::populateState("a.nom_name", "ASC");
	...
	protected function getListQuery()
	{
		...
		$query->select('nom.name AS nom_id, sec.name AS sec_id');
		...
		$query->select('pos.pos_name');
		$query->join('LEFT', '#__gavoting_positions AS pos ON pos.id = a.position_id');

		// Join over the category field 'cat_id'
		$query->select('s.title AS cat_id_name, CONCAT(s.title," - ", pos.pos_name) AS position_id_name');
		$query->join('LEFT', '#__categories AS s ON s.id = pos.cat_id');
		...
		$orderCol  = $this->state->get('list.ordering', "a.nom_name");
