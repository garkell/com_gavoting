...
class GavotingViewPosition extends HtmlView
{
	...
	protected function addToolbar()
	{
		...
		if (!$checkedOut && ($canDo->get('core.edit') || ($canDo->get('core.create')))) {
			ToolbarHelper::apply('position.apply', 'JTOOLBAR_APPLY');
			ToolbarHelper::save('position.save', 'JTOOLBAR_SAVE');
		}

		if (!$checkedOut && ($canDo->get('core.create'))) {
			ToolbarHelper::save2new('position.save2new', 'JTOOLBAR_SAVE_AND_NEW');
		}

		// If an existing item, can save to a copy.
		if (!$isNew && $canDo->get('core.create')) {
			ToolbarHelper::save2copy('position.save2copy', 'JTOOLBAR_SAVE_AS_COPY');
		}
