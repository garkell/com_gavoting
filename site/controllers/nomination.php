 ...
 * @since  1.6
 */
class GavotingControllerNomination extends BaseController
{
	/**
	...
	public function edit()
	{
		...
		// Get the model.
		$model = $this->getModel('Nomination', 'GavotingModel');
	...
	public function remove()
	{
		...
		if ($user->authorise('core.delete', 'com_gavoting'))
		{
			...
			else
			{
                ...
                $app->redirect(Route::_('index.php?option=com_gavoting&view=nominations', false));
			}
	...
	public function agree()
	{
		...
		if ($user->authorise('core.nominate', 'com_gavoting')) {
			$id = $app->input->getInt('id', 0);
			$app->setUserState('com_gavoting.edit.nomination.id', $id);
			...
			if ($return === false) {
                ...
                $app->enqueueMessage(Text::_('COM_GAVOTING_AGREED_NOMINATION'), 'message');
                $app->redirect(Route::_('index.php?option=com_gavoting&view=nominations', false));
			}
		} else {
			$app->enqueueMessage(Text::_('COM_GAVOTING_DECLINED_NOMINATION'), 'danger');
		}
