 ...
 * @since  1.6
 */
class GavotingControllerVoter extends BaseController
{
	/**
	...
	public function edit()
	{
		...
		// Get the model.
		$model = $this->getModel('Voter', 'GavotingModel');
	...
	public function publish()
	{
		...
		if ($user->authorise('core.edit', 'com_gavoting') || $user->authorise('core.edit.state', 'com_gavoting'))
		{
			...
			if (!$item)
			{
				...
				$this->setRedirect(Route::_('index.php?option=com_gavoting&view=voters', false));
			}
			else
			{
	...
	public function remove()
	{
		...
		if ($user->authorise('core.delete', 'com_gavoting'))
		{
			...
			if ($return === false)
			{
				$this->setMessage(Text::sprintf(Text::_('COM_GAVOTING_DELETE_FAILED'), $model->getError()), 'warning');
			}
			else
			{
                ...
                $app->redirect(Route::_('index.php?option=com_gavoting&view=voters', false));
			}
	...
	public function removeFile()
	{
		...
		if ($user->authorise('core.delete', 'com_gavoting'))
		{
			...
			if ($return === false) {
                ...
                $app->enqueueMessage(Text::_('COM_GAVOTING_FILE_DELETED_SUCCESSFULLY'), 'success');
                $app->redirect(Route::_('index.php?option=com_gavoting&view=voters', false));
				// Check in the profile.
				$model->checkin($id);
			}
