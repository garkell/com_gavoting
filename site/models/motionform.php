 ...
 * @since  1.6
 */
class GavotingModelMotionForm extends FormModel
{
    private $item = null;
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_gavoting.motion', 'motionform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $data = Factory::getApplication()->getUserState('com_gavoting.edit.motion.data', array());
    ...
    {
        $id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('motion.id');
    ...
    public function delete($pk)
    {
        ...
        if (empty($pk)) {
            $pk = (int) $this->getState('motion.id');
        }
     ...
     * @param   int $pk Item primary key
     */
    public function voteFor($pk)
    {
        $user = Factory::getUser();
        $hasVoted = GavotingHelper::hasVoted($user->id, $pk);
        ...
        if ($user->authorise('core.vote', 'com_gavoting') !== true) {
            throw new Exception(Text::_('JERROR_ALERTNOAUTHOR'), 403);
        }

        if ($hasVoted) {
            throw new Exception(Text::_('COM_GAVOTING_MOTION_ALREADY_VOTED'), 404);
        }

        $table = $this->getTable();
        $table->load($pk);
        $table->votes_for = $table->votes_for + 1;

        if ($table->store($pk) !== true) {
            throw new Exception(Text::_('JERROR_FAILED'), 501);
        }

        $voteCreated = GavotingHelper::createVote($user->id, $pk);
     ...
     * @param   int $pk Item primary key
     */
    public function voteAgainst($pk)
    {
        $user = Factory::getUser();
        $hasVoted = GavotingHelper::hasVoted($user->id, $pk);
        ...
        $table = $this->getTable();
        $table->load($pk);
        $table->votes_against = $table->votes_against + 1;
     ...
     * @param   int $pk Item primary key
     */
    public function voteDecision($pk)
    {
        $user = Factory::getUser();
        ...
        if ($table->votes_for != $table->votes_against) {
            ...
            throw new Exception(Text::_('COM_GAVOTING_CAST_DECIDER'), 403);
            //Factory::getApplication()->enqueueMessage(Text::_('COM_GAVOTING_CAST_DECIDER'),'danger');
			//return $pk;
        }
