 ...
 * @since  1.6
 */
class GavotingModelPositionForm extends FormModel
{
    private $item = null;
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_gavoting.position', 'positionform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('position.id');
    ...
    public function delete($pk)
    {
        ...
        if (empty($pk)) {
            $pk = (int) $this->getState('position.id');
        }
