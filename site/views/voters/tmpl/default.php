...
<form action="<?php echo htmlspecialchars(Uri::getInstance()->toString()); ?>" method="post"
	...
	<table class="table table-striped" id="voterList">
		...
		<tr>
			...
			<th class=''>
				<?php echo Text::_('COM_GAVOTING_VOTERS_USER_ID'); ?>
			</th>
			<th class='center'>
				<?php echo Text::_('COM_GAVOTING_VOTERS_CAT_MOTION'); ?>
			</th>
			<th class='center'>
				<?php echo Text::_('COM_GAVOTING_VOTERS_PROXY_VOTE'); ?>
			</th>
		...
		<?php foreach ($this->items as $i => $item) : ?>
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<?php if ($canEdit || $canDelete): ?>
					<td class="center">
						<?php if ($canEdit): ?>
							<a href="<?php echo Route::_('index.php?option=com_gavoting&task=voterform.edit&id=' . $item->id, false, 2); ?>" class="btn btn-mini" type="button" title="Edit Record"><i class="icon-edit" ></i></a>
						<?php endif; ?>
						<?php if ($canDelete): ?>
							<a href="<?php echo Route::_('index.php?option=com_gavoting&task=voterform.remove&id=' . $item->id, false, 2); ?>" class="btn btn-mini delete-button" type="button" title="Delete Record"><i class="icon-trash" ></i></a>
							<a href="<?php echo Route::_('index.php?option=com_gavoting&task=voterform.archive&id=' . $item->id, false, 2); ?>" class="btn btn-mini" type="button" title="Archive Record"><i class="icon-archive" ></i></a>
						<?php endif; ?>
					</td>
				<?php endif; ?>
